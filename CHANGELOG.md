...
- **Fix: subagents on a `claude-bridge/*` model failed with `400 out of extra usage`** — pi's system prompt widens after `before_agent_start`: verbose MCP tool descriptions merge into it only once their servers connect. So the prompt the provider actually queries with — the one pi-subagents embeds verbatim into a child via `ctx.getSystemPrompt()` at dispatch — is larger than what was captured at `before_agent_start` (measured 23,479 vs 10,988 chars, diverging at the tool list). The child's turn then resolved against no capture, fell to a verbatim side request, and shipped pi's harness, tripping the server's content-dependent plan-eligibility check. The bridge now also records `ctx.getSystemPrompt()` at `agent_start` — the fully-widened prompt, captured just before the query — so a subagent embedding it resolves on the main lane with the harness stripped. Covered by `tests/unit-agent-start-capture.mjs`.
