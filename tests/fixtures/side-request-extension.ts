	...
	async function runSideRequest(model: unknown, outPath: string, word: string) {
		...
		try {
			const stream = agentLoop(
				...
				// pi-agent-core >= 0.87: the system prompt is the transcript's leading system message.
				{ messages: [{ role: "system", content: NOTE_TAKER_SYSTEM, timestamp: Date.now() }], tools: [recordNote] },
